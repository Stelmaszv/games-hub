...
export class PublishersMainListComponent {
  ...
  ngOnInit(): void {
    ...
    this.isGrantedService.setData('CAN_SHOW_PUBLISHER', { "entity": "Publisher","id":69 });
    
    this.isGrantedService.responseData.subscribe((el) => {
      this.canShowPublisher = el
    });

    this.isGrantedService.setData('CAN_EDIT_PUBLISHER', { "entity": "Publisher","id":69 });
    
    this.isGrantedService.responseData.subscribe((el) => {
      this.canEditPublisher = el
    });

    this.isGrantedService.setData('CAN_DELETE_PUBLISHER', { "entity": "Publisher", "id":69 });
